...
def update_user_db(key, uname, cpass):
    ...
    try:
        if key != uname:
            cur.execute("UPDATE USERS SET c_username=?, c_pass=?, WHERE c_username=?",
                        (uname, cpass, key))
        else:
            cur.execute("UPDATE USERS SET c_pass=? WHERE c_username=?", (cpass, uname))
        db.commit()
        db.close()
        return True
    except sqlite3.IntegrityError:
...
def add_prod_to_trans(trans_id, prod_id, prod_price, quantity, discount):
    ...
    try:
        ...
        cur.execute("SELECT item_quantity FROM TRANSACTED_PRODUCTS WHERE (transact_id=? AND product_id=?)",
                    (trans_id, prod_id))
